...
RunNetMHCIIpan_ng <- function(alleles,
  ...
  alleles_supported <- unlist(strsplit(readLines(paste0(c(software_path_splt[-length(software_path_splt)],
                                                          "data", "allelelist.txt"), collapse = "/")), spltchar))
  ...
  if(!is.null(value_type)) {
    ...
    outobj <- CollectBindingResults(results, hla_type = 2, value_type = value_type, output_format = output_format, version_number = version_number)
