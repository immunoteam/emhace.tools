...
RunNetMHCpan <- function(alleles,
  ...
  # splitting the peptides into separate pepfiles
  if(!paired_input) {
    # creating pepfiles
    peptides_per_threads <- data.frame(peptides, thread = cut(seq_along(peptides), threads, labels = FALSE))
    ...
    inputdf$thread <- map_chr(strsplit(inputdf$Var1, "\\."), ~tail(.x, 1))
    ...
    if(!is.null(result_files_location)) {
      ...
      inputdf$alleles_underscore <- gsub("\\:", "_", inputdf$Var2) # windows cannot handle colons in filenames
